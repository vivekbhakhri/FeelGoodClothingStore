...
// if (document.querySelector('input[name="pop"]')) {
// 	document.querySelectorAll('input[name="pop"]').forEach((elem) => {
...
//   }
	...
	$('body').delegate('#price_sort1','click',function(e){
		debugger
		e.preventDefault();
		$(this).css('color','red');
		$('#pop_sort').css('color','black');
		$.ajax({
			url: "action.php",
			method: "POST",
			data: {getProduct:1,price_sorted1:1},
			success: function(data){
				$('#get_product').html(data);
			}
		})
